...
int main() {
	...
	cout << "Second I would like to test if we can count a sum using a normal loop..." << endl;
	...
	cout << "Third I would like to test if we can count a sum using recursion.." << endl;

	//This second point is for recursion
	auto startHigh3 = std::chrono::high_resolution_clock::now();     // Timing for sumArray assertions
	assert(RsumArrayR(arr, 0) == 0); // Sum of an empty range
	assert(RsumArrayR(arr, 1) == 1); // Sum of first element
	assert(RsumArrayR(arr, 2) == 3); // Sum of first two elements (1 + 2)
	assert(RsumArrayR(arr, 5) == 15); // Sum of all elements (1 + 2 + 3 + 4 + 5)
	auto endHigh3 = std::chrono::high_resolution_clock::now();    //End of Timing
	std::chrono::duration<double> elapsed3 = endHigh3 - startHigh3;

	//Ouput for user
	cout << "All program tests passed!" << endl;
	cout << "Time taken by sumArray tests: " << elapsed3.count() << " seconds" << endl;
